...
After #536 shadow parity, the normal compact operator path for proven
validator-invocation scopes is catalog-backed WGCF:
...
catalog scope unless WGCF is blocked, the target scope is not admitted, or the
owner repo is being validated before a source landing.
